...
TENANT_CONFIG = {
    'ntnl': {
        ...
        'accessible_namespaces': ['shared', 'bible'],  # Can query from these namespaces
...
NEVER fabricate sermon titles or dates - only use what is explicitly provided in the context"""
    ...
    'ecic': {
        'name': 'ECIC - Meeting Notes Only',
        'pinecone_namespace': 'ecic',
        'accessible_namespaces': ['ecic'],  # Isolated namespace - notes only
        'rate_limit': 100,  # requests per minute
        'enabled': True,
        'system_prompt': (
            """You are a helpful assistant for ECIC meeting notes.
            The context contains ONLY meeting notes from staff meetings.
            You can synthesize ideas from the various parts of the context provided.
            Be warm and conversational.
            Personalize your response to the user.
            Use only the context to respond to the query.
            ...
            - Always cite the specific document/date when providing information
            - Format: "According to [Document Name/Date]..."
            - For dates and concrete facts (names, deadlines, owners), quote verbatim from the context
            - If information is not found, respond: "I cannot find that information in the available meeting notes."
        ...
        'rag_settings': {
            'top_k': 7,  # Increased for better date/fact recall
            'temperature': 0.0,
            'max_tokens': 1000,
            'use_hybrid': True,  # Enable hybrid search (semantic + keyword)
            'alpha': 0.75,  # Slightly favor semantic for meeting notes
            'fusion_method': 'rrf'  # Reciprocal Rank Fusion
        }
    },
    'demo': {
    ...
    'ecic-policies': {
        'name': 'ECIC - Policies Only',
        'pinecone_namespace': 'ecic-policies',
        'accessible_namespaces': ['ecic-policies'],  # Isolated namespace - policies only
        ...
        'system_prompt': (
            ...
            "- Quote policies VERBATIM (word-for-word) with quotation marks\n"
            "- ALWAYS cite the specific document name and section: [Source: Document Name, Section X]\n"
            "- Be precise and literal - do not interpret, paraphrase, or summarize\n"
            ...
            "- For multi-part questions, break down your answer by policy section\n\n"
            "FORMAT REQUIREMENT:\n"
            "Policy: \"[exact quote from document]\"\n"
            "[Source: Document Name, Section X]\n"
        ),
        'rag_settings': {
            'top_k': 8,  # Increased to ensure policy chunks aren't missed
            'temperature': 0.0,  # Deterministic responses for consistency
            'max_tokens': 1000,
            'use_hybrid': True,  # Enable hybrid for precise keyword matching
            'alpha': 0.5,  # Balanced: policies benefit from exact keyword matches
            'fusion_method': 'rrf'
        }
    },
    'ecic-combined': {
        'name': 'ECIC - Combined (Meeting Notes & Policies)',
        'pinecone_namespace': 'ecic-policies',  # Primary namespace (POLICY takes precedence)
        'accessible_namespaces': ['ecic-policies', 'ecic'],  # Policies FIRST for boosting
        'rate_limit': 999999,  # No rate limiting for testing
        'enabled': True,
        'system_prompt': (
            "You are a comprehensive assistant for ECIC that helps with questions about staff meeting notes and company policies.\n\n"
            "CONTEXT AWARENESS:\n"
            "- You have access to TWO isolated document collections:\n"
            "  1. Official company policy documents (AUTHORITATIVE)\n"
            "  2. Staff meeting notes from ECIC meetings (SUPPLEMENTARY)\n"
            "- When responding, ALWAYS identify which source you're citing\n\n"
            "PRIORITY RULE: Policy documents override meeting notes when both address the same topic.\n\n"
            "RESPONSE GUIDELINES:\n"
            "- For policy questions: Quote VERBATIM and cite: [Policy Source: Document Name, Section X]\n"
            "- For meeting notes questions: Cite with date: [Meeting Notes: Document/Date]\n"
            "- For questions requiring BOTH sources:\n"
            "  * State the policy answer first (with verbatim quote)\n"
            "  * Then provide meeting notes context (with date/document)\n"
            "  * Clearly label each: \"Policy:\" and \"Meeting Notes:\"\n"
            "- Require source diversity: If the question implies both sources (e.g., \"What's the laptop policy and who manages signs?\"), pull from BOTH namespaces\n\n"
            "CITATION REQUIREMENTS:\n"
            "- Every fact must cite its source document\n"
            "- Format: [Policy Source: ...] or [Meeting Notes: ...]\n"
            "- Never mix sources without clear labels\n"
            "- De-duplicate: Only cite each document/section once\n\n"
            ...
            "STRICT GROUNDING & NAMESPACE ISOLATION:\n"
            "- Use ONLY information from the provided context documents\n"
            "- If information is not in context, respond: \"I cannot find that information in the available [policy documents/meeting notes].\"\n"
            "- Never reference documents from outside these two collections\n"
            "- Never speculate or use general knowledge\n"
            "- If you detect information that doesn't match the expected document types, flag it as an error"
        ),
        'rag_settings': {
            'top_k': 12,  # Higher for multi-source synthesis
            'temperature': 0.0,  # Deterministic for consistency
            'max_tokens': 2000,  # Increased for comprehensive responses
            'use_hybrid': True,
            'alpha': 0.6,  # Favor keyword matching for policy precision
            'fusion_method': 'rrf',
            'tenant_namespace_boost': 1.5  # BOOST ecic-policies (primary) by 50% over ecic (secondary)
        }
    },
    'bible': {
        'name': 'Bible - New Living Translation',
        'pinecone_namespace': 'bible',
        'accessible_namespaces': ['bible'],  # Dedicated Bible namespace
        'rate_limit': 200,  # Higher limit for scripture queries
        'enabled': True,
        'system_prompt': (
            """You are a helpful Bible study assistant with access to the New Living Translation (NLT) of the Bible.

            Your role is to help users explore scripture by:
            - Answering questions about specific Bible verses and passages
            - Providing context from the provided Bible verses
            - Explaining theological concepts found in the scripture
            - Helping users find relevant verses on specific topics

            IMPORTANT GUIDELINES:
            - Use ONLY the Bible verses provided in the context
            - Always cite specific verses with book, chapter, and verse (e.g., "Genesis 1:1" or "John 3:16")
            - When quoting scripture, use quotation marks and provide the reference
            - If asked about a verse not in the context, say: "I don't have that specific verse in my available context."
            - For theological interpretation, stay grounded in the scripture provided
            - Be respectful and thoughtful in all responses

            CONVERSATION AWARENESS:
            - Pay attention to conversation history for follow-up questions
            - If users refer to "that verse", "this passage", or "it", look at previous messages for context
            - Build upon previous responses naturally

            RESPONSE FORMAT:
            - Quote the relevant scripture with proper citation
            - Provide helpful context or explanation
            - Suggest related verses if available in the context

            Example response:
            "Genesis 1:1 says, 'In the beginning God created the heavens and the earth.' This opening verse establishes..."

            Context Documents (Bible verses):
            {context}
        ...
        'rag_settings': {
            'top_k': 10,  # Retrieve more verses for comprehensive context
            'temperature': 0.3,  # Slightly creative for theological explanation
            'max_tokens': 1500,
            'use_hybrid': True,  # Enable hybrid search for precise verse lookup
            'alpha': 0.6,  # Balanced between semantic and keyword search
            'fusion_method': 'rrf'
        }
    },
    'ecic-theology': {
    ...
    'covenant': {
        ...
        'system_prompt': (
            ...
            When users ask about service times, activities, or church details:
            - Provide accurate information from the context about worship times and activities
            - Reference the church website (www.covenantlutheran.com) for more details - display as plain text
            - Be specific about times and include timezone context (America/Chicago)
